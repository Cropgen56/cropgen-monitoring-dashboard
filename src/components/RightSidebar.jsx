...
export default function RightSidebar({
  ...
  const [snapshots, setSnapshots] = useState([]); // empty for now

  const selectedCropFields = useMemo(() => {
    if (!Array.isArray(farms) || !selectedCrop) return [];
    return farms.filter((farm) => farm.cropName === selectedCrop);
  }, [farms, selectedCrop]);
  ...
  const availableCrops = useMemo(() => {
    ...
    return [
      ...new Set(farms.map((farm) => farm.cropName).filter(Boolean)),
  ...
  return (
    <aside className="w-full lg:bg-[#132f1e] p-3 sm:p-4">
      <div className="bg-[#0C2214] rounded-xl px-4 sm:p-5 shadow-lg">
        ...
        <div className="mt-4">
          ...
          <select
            ...
            className="w-full mt-1 bg-cg-panel text-white rounded-lg px-3 py-2 text-xs border border-green-500/20"
          >
        ...
        {/* <div className="text-center py-6">
          <div className="text-gray-400 text-sm">No saved fields yet</div>
            ...
            <div className="space-y-3">
              {savedFields.map((field) => (
                <div
                  ...
                  className={`rounded-lg p-3 flex justify-between cursor-pointer border transition-all duration-500 ease-in-out
                    ${
                  ...
                  <button
                    ...
                    className="text-red-400 hover:text-red-500 transition-all duration-500 ease-in-out cursor-pointer"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
